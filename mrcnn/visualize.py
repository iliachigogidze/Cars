...
def display_instances(image, boxes, masks, class_ids, class_names,
    ...
    for i in range(N):
        ...
        y1, x1, y2, x2 = boxes[i]
        if show_bbox:
            p = patches.Rectangle((x1, y1), x2 - x1, y2 - y1, linewidth=0,
        ...
        for verts in contours:
            ...
            p = Polygon(verts, facecolor='w', edgecolor='none')
            ax.add_patch(p)
...
def save_image(image, image_name, boxes, masks, class_ids, scores, class_names, filter_classs_names=None,
    ...
    if mode != 1:
        for index, value in enumerate(useful_mask_indices):
            try:
                masked_image = apply_mask(masked_image, masks[:, :, value], (1.0, 1.0, 1.0))
            except:
                masked_image = apply_mask(masked_image, masks[:, :, value], (0.0, 0.0, 1.0))
